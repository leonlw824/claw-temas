  ...
  if (url.pathname.startsWith('/api/agents/') && req.method === 'GET') {
    ...
    if (parts.length === 2 && parts[1] === 'soul') {
      try {
        ...
        const content = await readAgentBootstrapFile(agentId, 'SOUL.md');
    ...
    if (parts.length === 2 && parts[1] === 'behavior') {
      try {
        const agentId = decodeURIComponent(parts[0]);
        const content = await readAgentBootstrapFile(agentId, 'AGENTS.md');
        sendJson(res, 200, { success: true, content });
      } catch (error) {
        sendJson(res, 500, { success: false, error: String(error) });
      }
  ...
  if (url.pathname.startsWith('/api/agents/') && req.method === 'PUT') {
    ...
    if (parts.length === 2 && parts[1] === 'model') {
      try {
        ...
        const snapshot = await updateAgentModel(agentId, body.model);
    ...
    if (parts.length === 2 && parts[1] === 'soul') {
      try {
        ...
        await writeAgentBootstrapFile(agentId, 'SOUL.md', body.content);
    ...
    if (parts.length === 2 && parts[1] === 'behavior') {
      try {
        const agentId = decodeURIComponent(parts[0]);
        const body = await parseJsonBody<{ content: string }>(req);
        await writeAgentBootstrapFile(agentId, 'AGENTS.md', body.content);
        sendJson(res, 200, { success: true });
      } catch (error) {
        sendJson(res, 500, { success: false, error: String(error) });
      }
