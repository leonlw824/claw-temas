...
export interface GatewayManagerEvents {
  ...
  'chat:history': (data: unknown) => void;
}
  ...
  private handleMessage(message: unknown): void {
    ...
    if (msg.type === 'res' && typeof msg.id === 'string') {
      if (msg.ok === false || msg.error) {
        ...
        // Emit chat:response event for task executor if payload contains messages
        if (
          msg.payload &&
          typeof msg.payload === 'object' &&
          'messages' in msg.payload &&
          Array.isArray((msg.payload as Record<string, unknown>).messages)
        ) {
          this.emit('chat:history', msg.payload);
        }

        if (resolvePendingGatewayRequest(this.pendingRequests, msg.id, msg.payload ?? msg)) {
          return;
        }
      }
    }
