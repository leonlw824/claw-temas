            ...
            {agents.map((agent) => (
              <AgentCard
                ...
                onChat={() => {
                  ...
                  const existingSession = getSessionForAgent(agent.id);
                  if (existingSession) {
                    // 如果存在，切换到该 session
                    switchSession(existingSession.key);
                  } else {
                    // 如果不存在，创建新的 session
                    const newKey = newSessionForAgent(agent.id);
...
function AgentCard({
  ...
  return (
    <div
      ...
      className={cn(
        'group flex items-start gap-4 p-4 rounded-2xl transition-all text-left border relative overflow-hidden bg-transparent border-transparent hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer',
