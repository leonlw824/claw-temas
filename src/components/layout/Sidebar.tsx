...
function NavItem({ to, icon, label, badge, collapsed, onClick, indented = false }: NavItemProps) {
  return (
    <NavLink
      to={to}
      onClick={onClick}
  ...
  return (
    ...
    >
      ...
      <nav className="flex flex-col px-2 gap-0.5">
        ...
        {/* 系统设置分类 */}
        {!sidebarCollapsed && (
          <div className="px-2.5 pt-3 pb-1 text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-wide">
            {t('sidebar.categorySystemSettings')}
          </div>
        )}
