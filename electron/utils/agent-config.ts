...
/**
 * Read models.json from agent directory and extract all model IDs
 */
async function readAgentModels(agentDir: string): Promise<string[]> {
  try {
    const modelsPath = join(expandPath(agentDir), 'models.json');
    ...
    // If models.json doesn't exist or can't be read, return empty array
    return [];
  }
}
...
async function buildSnapshotFromConfig(config: AgentConfigDocument): Promise<AgentsSnapshot> {
  ...
  const agents: AgentSummary[] = await Promise.all(entries.map(async (entry) => {
    ...
    // Extract actual model value (string or primary from object)
    let actualModel: string | null = null;
    if (typeof entry.model === 'string' && entry.model.trim()) {
      actualModel = entry.model.trim();
    } else if (entry.model && typeof entry.model === 'object' && 'primary' in entry.model) {
      actualModel = String(entry.model.primary || '').trim() || null;
    }

    // Read available models from agent's models.json
    const agentDir = entry.agentDir || getDefaultAgentDirPath(entry.id);
    const models = await readAgentModels(agentDir);
...
export async function updateAgentModel(agentId: string, model: string): Promise<AgentsSnapshot> {
  const config = await readOpenClawConfig() as AgentConfigDocument;
  const { agentsConfig, entries } = normalizeAgentsConfig(config);
  const index = entries.findIndex((entry) => entry.id === agentId);
  if (index === -1) {
  ...
  await writeOpenClawConfig(config);
  logger.info('Updated agent model', { agentId, model });
  return buildSnapshotFromConfig(config);
}

export async function readAgentBootstrapFile(agentId: string, filename: string): Promise<string> {
  const config = await readOpenClawConfig() as AgentConfigDocument;
  const { entries } = normalizeAgentsConfig(config);
  const agent = entries.find((entry) => entry.id === agentId);
  if (!agent) {
    throw new Error(`Agent "${agentId}" not found`);
  }

  const workspace = expandPath(agent.workspace || (agent.id === MAIN_AGENT_ID ? getDefaultWorkspacePath(config) : `~/.openclaw/workspace-${agent.id}`));
...
export async function writeAgentBootstrapFile(agentId: string, filename: string, content: string): Promise<void> {
  const config = await readOpenClawConfig() as AgentConfigDocument;
  const { entries } = normalizeAgentsConfig(config);
  const agent = entries.find((entry) => entry.id === agentId);
  if (!agent) {
  ...
  logger.info('Wrote agent bootstrap file', { agentId, filename });
}
