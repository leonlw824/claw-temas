...
interface ChatState {
  ...
  newSession: () => void;
  newSessionForAgent: (agentId: string) => string;
  getSessionForAgent: (agentId: string) => ChatSession | undefined;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  getSessionForAgent: (agentId: string) => {
    const { sessions } = get();
    return sessions.find((s) => s.key.startsWith(`agent:${agentId}:`));
  },
