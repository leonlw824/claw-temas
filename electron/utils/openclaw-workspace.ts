...
const CLAWT_BEGIN = '<!-- clawteams:begin -->';
const CLAWT_END = '<!-- clawteams:end -->';
...
export function mergeClawXSection(existing: string, section: string): string {
  const wrapped = `${CLAWT_BEGIN}\n${section.trim()}\n${CLAWT_END}`;
  const beginIdx = existing.indexOf(CLAWT_BEGIN);
  const endIdx = existing.indexOf(CLAWT_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + CLAWT_END.length);
  }
...
export async function repairClawXOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(CLAWT_BEGIN);
      const endIdx = content.indexOf(CLAWT_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + CLAWT_END.length).trim();
      if (before === '' && after === '') {
        try {
